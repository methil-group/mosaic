<template>
  <div
    ...
    @click="handleSelect">
    <div
      class="w-full h-full bg-[var(--panel-bg)] rounded-[24px] overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-[var(--border-color)] flex flex-col"
      ...
      <div class="px-5 py-4 bg-[var(--panel-bg)] border-t border-[var(--border-color)]">
        <h3 class="mb-1 text-[14px] font-black tracking-[0.5px] text-[var(--text-main)] uppercase truncate">{{ workspace.name }}
        </h3>
        <span class="block text-[9px] font-bold text-[var(--text-dim)] mb-3 truncate font-mono opacity-60"
          :title="workspace.path">{{ displayPath }}</span>
        <div class="flex items-center justify-between">
          <span class="text-[10px] font-extrabold text-[var(--text-dim)] uppercase tracking-widest">{{ agents.length }} agent{{
    ...
    <button v-if="workspace.id !== 'default'"
      class="absolute top-2 right-2 w-6 h-6 bg-[var(--panel-bg)] text-red-500 rounded-lg border border-[var(--border-color)] flex items-center justify-center opacity-0 -translate-y-1 scale-75 group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100 hover:bg-red-500 hover:text-white transition-all duration-300 ease-[cubic-bezier(0.19,1,0.22,1)] shadow-lg z-10"
